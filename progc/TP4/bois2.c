...
char** copy(char** files){
	...
	dst=malloc(sizeof(char*)*NB_FILES);
	...
	dst[i]=malloc(BUFFER_SIZE*sizeof(char));
	strcpy(dst[i],lines[rnd]);
...
int main (void){
	...
	files[3] = FILE_4;
	srandom(time(NULL));
	FILE* flot=fopen(OUT_FILE,"w+");
	int i;
	for(i=0;i<NB_SENTENCES;i++){
		sentence=copy(files);
		int j;
		for(j=0;j<NB_FILES;j++){
			fputc(sentence[j],flot);
		
		}
	liberer(flot,NB_FILES);
	free(sentence);
	if(i<NB_FILES-1)
		fputc("\n",flot);
	}
